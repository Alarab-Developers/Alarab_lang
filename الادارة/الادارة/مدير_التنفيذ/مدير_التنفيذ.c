...
static int execute_block(const char *body)
{
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        if (cmd.type == CMD_REPEAT)
        {
            ...
            {
                داخل_حلقة++;   // ⬅️ قبل التنفيذ

                int r = execute_block(body);

                داخل_حلقة--;   // ⬅️ بعد التنفيذ
        ...
        if (cmd.type == CMD_IF_DEF)
        {
            ...
            for (int j = 0; j < count; j++)
            {
                ...
                if (math_eval(conditions[j], &result) == 0 && result != 0)
                {
                    ...
                    {
                        if (داخل_حلقة)
                            return 2; // ⬅️ الآن صحيح
                        break;
                    }
                    break; // تم تنفيذ الشرط الصحيح، نخرج من الحلقة
                }
            }
            continue; // تابع باقي الأوامر بعد if
        }
        ...
        if (result == 2) // كسر
        {
            if (داخل_حلقة)
                ...
                continue; // ⬅️ تجاهل الكسر خارج loop
        }
    }
    return 0;
}
